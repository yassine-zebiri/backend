<x-layout :pageTitle="$pageTitle">
    <div class="bg-white rounded-sm w-full p-5">
        ...
        <form action="/dashboard/restaurants/add/p/{{ $data->id }}" method="post" enctype="multipart/form-data" >
            ...
            <div class="grid grid-cols-3 p-4 gap-3">
                ...
                <div class="input-span">
                    <span >open time</span>
                    <input  type="time" name="open_time" value="{{old("open_time", $data->open_time) }}">
                    @error('open_time')
                        <span class="text-sm text-red-500 font-extralight">{{ $message }}</span>
                    @enderror
                </div>
                <div class="input-span col-span-1">
                    ...
                    <input  type="time" name="close_time" value="{{old("close_time", $data->close_time) }}">
                    @error('close_time')
                        <span class="text-sm text-red-500 font-extralight">{{ $message }}</span>
                    @enderror
                </div>
